        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginTop: 12 }}>
          <button onClick={() => {
            import('jspdf').then(({ jsPDF }) => {
              ...
              doc.setFontSize(16)
              doc.text('KSP CrimeIntel — Conversation Export', 14, 20)
              doc.setFont('helvetica', 'normal')
              doc.setFontSize(9)
              doc.text(`Generated: ${new Date().toLocaleString('en-IN')}`, 14, 28)
              doc.text(`Officer: ${user?.name || 'Unknown'} (${user?.badge || 'N/A'})`, 14, 33)
              doc.line(14, 36, 196, 36)

              let y = 42
              messages.forEach(m => {
                if (y > 270) { doc.addPage(); y = 20 }
                const role = m.role === 'user' ? 'OFFICER' : 'CRIMEINTEL AI'
                const time = new Date(m.timestamp || Date.now()).toLocaleTimeString('en-IN', { hour:'2-digit', minute:'2-digit' })
                doc.setFont('helvetica', 'bold')
                doc.setFontSize(9)
                doc.text(`[${time}] ${role}`, 14, y)
                y += 5
                doc.setFont('helvetica', 'normal')
                doc.setFontSize(8)
                const clean = m.content.replace(/\*\*/g, '').replace(/\*/g, '').replace(/`/g, '')
                ...
                if (m.total_records) {
                  doc.setFontSize(7)
                  doc.text(`[${m.total_records.toLocaleString()} records | viz: ${m.viz_type || 'none'}]`, 16, y)
                  y += 4
                }
                y += 3
              })
              ...
              doc.setFontSize(7)
              doc.text('CONFIDENTIAL — Karnataka State Police | Generated by KSP CrimeIntel', 14, 290)
              doc.save(`CrimeIntel_Chat_${new Date().toISOString().slice(0,10)}.pdf`)
            })
